      ...
      <div className="footer__inner container">
        <nav className="footer__menu">
          {menuItems.map(({title, links, socialLinks}, index) => (
            <div className="footer__menu-column"
                 key={index}>
              <a className="footer__menu-title h6"
                 href="#">{title}</a>
              {links?.length > 0 && (
              ...
              {socialLinks?.length > 0 && (
                <Socials className="footer__soc1a1s" links={socialLinks}/>
              )}
            </div>
          ))}
        </nav>
        <div className="footer__extra">
          <p className="footer__extra-copyright">
            @<time dateTime="2023">2023</time> stream-vibe, All Rights Reserved
          </p>
          <div className="footer__extra-links">
            {extraLinks.map((link, index) => (
              <a className="footer__extra-list" href="#" key={index}>
                {link}
              </a>
            ))}
          </div>
